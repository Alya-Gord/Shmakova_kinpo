...
namespace Shmakova_tests
{
    ...
    public:
        ...
        TEST_METHOD(ValidDimensionsNormal) {
            FileIOHandler handler; int cols = 0, rows = 0;
            handler.parseDimensions("5 10", cols, rows);
            Assert::AreEqual(5, cols); Assert::AreEqual(10, rows);
        }
        ...
        TEST_METHOD(ValidDimensionsMax) {
            ...
            Assert::AreEqual(100, cols); Assert::AreEqual(100, rows);
        }
        ...
        TEST_METHOD(ValidDimensionsMin) {
            ...
            Assert::AreEqual(1, cols); Assert::AreEqual(1, rows);
        }
        ...
        TEST_METHOD(ThreeInputsError) {
            FileIOHandler handler; int cols = 0, rows = 0;
            try { handler.parseDimensions("10 20 30", cols, rows); Assert::Fail(); }
           catch (const Error& e) { Assert::IsTrue(e.type == DIMENSION_TOO_MANY_NUMBERS); }
        }
        ...
        TEST_METHOD(OneInputError) {
            FileIOHandler handler; int cols = 0, rows = 0;
            try { handler.parseDimensions("10", cols, rows); Assert::Fail(); }
           catch (const Error& e) { Assert::IsTrue(e.type == DIMENSION_MISMATCH); }
        }
        ...
        TEST_METHOD(MoreThan100Error) {
            FileIOHandler handler; int cols = 0, rows = 0;
            try { handler.parseDimensions("101 50", cols, rows); Assert::Fail(); }
           catch (const Error& e) {
                Assert::IsTrue(e.type == OUT_OF_RANGE);
                Assert::AreEqual(101, e.incorrect_value);
            }
        }
        ...
        TEST_METHOD(ZeroDimensionError) {
            FileIOHandler handler; int cols = 0, rows = 0;
            try { handler.parseDimensions("0 10", cols, rows); Assert::Fail(); }
           catch (const Error& e) { Assert::IsTrue(e.type == DIMENSION_NEGATIVE); }
        }
        ...
        TEST_METHOD(NegativeDimensionError) {
            FileIOHandler handler; int cols = 0, rows = 0;
            try { handler.parseDimensions("10 -5", cols, rows); Assert::Fail(); }
           catch (const Error& e) { Assert::IsTrue(e.type == DIMENSION_NEGATIVE); }
        }
        ...
        TEST_METHOD(FloatDimensionError) {
            FileIOHandler handler; int cols = 0, rows = 0;
            try { handler.parseDimensions("5.5 10", cols, rows); Assert::Fail(); }
           catch (const Error& e) { Assert::IsTrue(e.type == DIMENSION_NOT_INTEGER); }
        }
        ...
        TEST_METHOD(GarbageDimensionError) {
            FileIOHandler handler; int cols = 0, rows = 0;
            try { handler.parseDimensions("abc 10", cols, rows); Assert::Fail(); }
           catch (const Error& e) { Assert::IsTrue(e.type == DIMENSION_NOT_INTEGER); }
        }
    };
}