...
public:
    /*!
     * \brief Метод проверки расширения файла на соответствие заданному множеству расширений
     * \param[in] filename путь файла, расширение которого нужно проверить
     * \param[in] extensionsBox множество допустимых расширений
    ...
    /*!
     * \brief Метод валидации полученных значений
    ...
    /*!
     * \brief Метод чтения содержимого текстового файла
    ...
    /*!
     * \brief Метод парсинга строки для получения размерностей матрицы
    ...
    /*!
     * \brief Метод чтения размерностей и данных матрицы из списка строк
    ...
    /*!
     * \brief Метод вывода результата в файл
     * \param[in] filename имя файла, в который нужно записать результат
     * \param[in] matrix матрица, из которой берется подматрица
     * \param[in,out] res ссылка на структуру с параметрами максимальной подматрицы для вывода
     */
